...
res.redirect("https://t.me/AK_BOTS_TAME");
}
...
( Many sites may not work under this method if they have x-frame header present.Ex https://google.com )
);
}
...
if(use1pt){
var x=await fetch(https://shorts-links-api.vercel.app/?${encodeURIComponent(cUrl)}).then(res => res.json());
var y=await fetch(https://shorts-links-api.vercel.app/${encodeURIComponent(wUrl)}).then(res => res.json());
